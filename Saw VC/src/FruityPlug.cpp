...
// It should be exported from the dll (usually through a module definition file
// (.def)
extern "C" TFruityPlug *_stdcall CreatePlugInstance(TFruityPlugHost *Host,
  ...
  if (Host->HostVersion < 006000000) {
    // format error buffer
    char error_buf[256];
    sprintf_s(error_buf, 256,
              "Saw VC|Error: FL Studio 6 or newer is required. (Current "
              "Version Id: %d)",
              Host->HostVersion);
    // example: show error message box through FL
    Host->Dispatcher(Tag, FHD_MsgBox, (intptr_t)error_buf,
                     MB_OK | MB_ICONERROR);
...
BOOL APIENTRY DllMain(HANDLE hModule, DWORD ul_reason_for_call,
  ...
  case DLL_PROCESS_ATTACH:
    HInstance = (HINSTANCE)hModule; // save global instance handle. HInstance is
                                    // defined in fp_plugclass.cpp
    break;
