...
std::string LongName =
    "Saw (VC++)"; // full plugin name (should be the same as DLL name)
std::string ShortName = "Saw"; // & short version (for labels);

// the information structure describing this plugin to the host
TFruityPlugInfo PlugInfo = {
    CurrentSDKVersion,
    (char *)(LongName.c_str()),
    (char *)(ShortName.c_str()),
    FPF_Type_FullGen,
    NumParamsConst,
...
LRESULT CALLBACK PlgWndProc(HWND hwnd, UINT message, WPARAM wParam,
                            LPARAM lParam) {
  // get plugin pointer
...
TTestPlug::TTestPlug(int SetHostTag, TFruityPlugHost *SetPlugHost,
                     HINSTANCE SetInstance)
    : TCPPFruityPlug(SetHostTag, SetPlugHost, SetInstance), VoiceList() {
  Info = &PlugInfo;
...
intptr_t _stdcall TTestPlug::Dispatcher(intptr_t ID, intptr_t Index,
                                        intptr_t Value) {
  ...
  case FPD_ShowEditor:
    ...
    } else // show
    {
      EditorHandle = CreateWindowEx(
          0, "SawVCExampleWindow", "SawVCExampleWindow", WS_CHILD | WS_VISIBLE,
          0, 0, 300, 200, (HWND)Value, NULL, HInstance, NULL);
...
void _stdcall TTestPlug::SaveRestoreState(IStream *Stream, BOOL Save) {
  if (Save) {
    // first write a version number, so we can change the format later on if we
    // want to
    ...
    if (version <=
        PlugVersion) // we know what to do if the version is older than our own
    {
...
void _stdcall TTestPlug::GetName(int Section, int Index, int Value,
                                 char *Name) {
  if (Section == FPN_Param) {
    switch (Index) {
    case paramVolume:
      strcpy_s(Name, 256, "Volume");
    }
  }
}

int _stdcall TTestPlug::ProcessParam(int ThisIndex, int ThisValue,
                                     int RECFlags) {
  if ((RECFlags & REC_FromMIDI) == REC_FromMIDI)
    ThisValue = ThisValue * 100 /
                65536; // REC_FromMIDI means the value is between 0 and 65536.
                       // Here we convert it to our own range (volume parameter
  ...
  return ThisValue; // return the value (in case of REC_FromMIDI or
                    // REC_GetValue)
}

void _stdcall TTestPlug::Gen_Render(PWAV32FS DestBuffer, int &Length) {
  float volume =
      ParamValue[paramVolume] / 100.f; // get the volume from 0 to 1.0
  float LVol, RVol;

  PWAV32FM Buffer =
      (PWAV32FM)
          PlugHost->TempBuffers[0]; // get our temporary buffer from the host
  ...
  for (it = VoiceList.begin(); it != VoiceList.end(); ++it) {
    ...
    // let the host compute volumes taking into account the per-voice pan and
    // volume
    else
      PlugHost->ComputeLRVol(LVol, RVol, v->Params->FinalLevels.Pan,
                             v->Params->FinalLevels.Vol);
    ...
    for (int j = 0; j < Length; j++) {
      ...
      *((float *)Buffer + j) = *((float *)(SawTable + pos)) * volume;
      // !!! make sure to disable any overflow checking in the compiler, as this
      // is based on an overflow trick
      v->Position += v->Speed;
    }
...
TVoiceHandle _stdcall TTestPlug::TriggerVoice(PVoiceParams VoiceParams,
                                              intptr_t SetTag) {
  ...
  // we use the voice class' pointer for this, since it's supposed to be unique
  // within the process
  return (TVoiceHandle)v;
}

void _stdcall TTestPlug::Voice_Release(TVoiceHandle Handle) {
  // handle is the value we returned in TriggerVoice, so it's really a pointer
  // to a TVoice object
...
int _stdcall TTestPlug::Voice_ProcessEvent(TVoiceHandle Handle, int EventID,
