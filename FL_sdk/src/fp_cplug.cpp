...
const float Ramp_Res_Unit =
    1.0f /
    1000.0f; // will take this number of samples to go from 0 to 1 at 44100Hz
const float Ramp_Res_Norm = Ramp_Res_Unit; // source is -1..1 normalized
const float Ramp_Res_Min =
    Ramp_Res_Unit * 10.0f; // source is -1..1 normalized, but quicker ramping,
                           // with minimum safety
...
int TAudioRenderer::getSmpRate() { return fSmpRate; }
...
// create the object
TCPPFruityPlug::TCPPFruityPlug(intptr_t SetHostTag,
                               TFruityPlugHost *SetPlugHost, HINSTANCE instance)
    : TFruityPlug() {
  HostTag = SetHostTag;
...
#ifndef __APPLE__
  char *path =
      (char *)(PlugHost->Dispatcher(HostTag, FHD_GetPath, GP_ProgPath, 0));
...
bool TCPPFruityPlug::AllocTempBuffer(int Length, int NumBuffers) {
  if (Length > MaxProcessLength) {
    ...
    ProcessTempBuffer =
        (PWAV32FM)realloc(ProcessTempBuffer, MaxProcessLength * 8 * NumBuffers);
    return true;
  } else
    return false;
}
...
void TCPPFruityPlug::ProcessAllParams() {
  for (int n = 0; n < Info->NumParams; n++)
    ProcessParam(n, ProcessParam(n, 0, REC_GetValue),
                 REC_UpdateValue | REC_UpdateControl);
}

// to safely skip a rendering (for plugins that don't render)
void TCPPFruityPlug::SkipRendering(void *SourceBuffer, void *DestBuffer,
                                   int Length) {
  if (SourceBuffer != DestBuffer)
    memmove(DestBuffer, SourceBuffer, Length << 3);
}

// get the speed step to go through the LFO table at a given pitch (cents)
// (tuned around C5)
...
// hints
void TCPPFruityPlug::ShowHintMsg(char *Msg) { PlugHost->OnHint(HostTag, Msg); }
...
// const PitchTypeT:Array[-1..2] of String=(' octave',' semitone',' cent','
// Hz');

void TCPPFruityPlug::ShowHintMsg_Pitch(int Value, int PitchType) {
  std::vector<std::string> PitchTypeT(4);
  PitchTypeT[0] = "octave";
  ...
  char Msg[256];
  char M2[256];
  sprintf(M2, "%d %s", Value,
          PitchTypeT[PitchType + 1].c_str()); // PitchType = -1..2
...
void TCPPFruityPlug::ShowHintMsg_Pitch(float Value, int PitchType, int Digits) {
  ...
  sprintf(M2, M2, Value, PitchTypeT[PitchType + 1].c_str());
...
void TCPPFruityPlug::CheckItem(HMENU Menu, int Item, int ParamNum, int Index) {
  intptr_t funcresult =
      PlugHost->Dispatcher(HostTag, FHD_GetParamMenuFlags, ParamNum, Index);

  unsigned int check = 0;
#ifndef __APPLE__
  if ((funcresult & FHP_Checked) != 0)
    check = MF_CHECKED;
  else
    check = MF_UNCHECKED;
  CheckMenuItem(Menu, Item, MF_BYPOSITION | check);

  if ((funcresult & FHP_Disabled) == 0)
    check = MF_ENABLED;
  else
    check = MF_DISABLED;
...
#ifdef __BORLANDC__
void TCPPFruityPlug::AdjustParamPopup(TMenuItem *Item, int ParamNum,
                                      int FirstItemIndex) {
  if (!Item)
    return;
  ...
  do {
    // get menu entry
    MenuEntry = (PParamMenuEntry)(PlugHost->Dispatcher(
        HostTag, FHD_GetParamMenuEntry, ParamNum, n));
    if (MenuEntry) {
...
void TCPPFruityPlug::AdjustParamPopup(HMENU Item, int ParamNum,
                                      int FirstItemIndex, int FirstID)
...
intptr_t _stdcall TCPPFruityPlug::Dispatcher(intptr_t ID, intptr_t Index,
                                             intptr_t Value) {
  intptr_t v = 0;

  switch (ID) {
  case FPD_UseIncreasedMIDIResolution:
    v = 1;
  }
...
void _stdcall TCPPFruityPlug::Eff_Render(PWAV32FS SourceBuffer,
                                         PWAV32FS DestBuffer, int Length) {}

void _stdcall TCPPFruityPlug::Gen_Render(PWAV32FS DestBuffer, int &Length) {}

void _stdcall TCPPFruityPlug::GetName(int Section, int Index, int Value,
                                      char *Name) {}
...
TVoiceHandle _stdcall TCPPFruityPlug::TriggerVoice(PVoiceParams VoiceParams,
                                                   intptr_t SetTag) {
  return 0;
}
...
int _stdcall TCPPFruityPlug::Voice_ProcessEvent(TVoiceHandle Handle,
                                                intptr_t EventID,
                                                intptr_t EventValue,
                                                intptr_t Flags) {
  return 0;
...
int _stdcall TCPPFruityPlug::Voice_Render(TVoiceHandle Handle,
                                          PWAV32FS DestBuffer, int &Length) {
  return 0;
}

int _stdcall TCPPFruityPlug::ProcessEvent(int EventID, int EventValue,
                                          int Flags) {
  return 0;
}
